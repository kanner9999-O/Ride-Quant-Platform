...
2. Đọc [`/docs/constitution/02-platform-invariants.md`](../constitution/02-platform-invariants.md) — 13 Invariant KHÔNG được vi phạm dù bất kỳ lý do gì.
...
7. Nếu review evidence (ví dụ trong một ADR) pin một reviewer identity không khớp trực tiếp tên trong `team.yaml` (ví dụ `"Independent Review B"`), tra field `aliases` của từng member trong [`team.yaml`](./team.yaml) — đó là nguồn resolve DUY NHẤT cho alias ↔ actor mapping (xem [`roles.md`](./roles.md)).
