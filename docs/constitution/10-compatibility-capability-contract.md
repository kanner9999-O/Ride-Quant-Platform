...
Suy diễn ngược lại ("có version mới → activation lịch sử phải xét lại") phá immutable reference, phá independent versioning ([Chapter 9 §9.7](./09-plugin-model.md)), và sửa hồi tố eligibility của một boundary đã đóng. **Historical Compatibility Result giữ nguyên vĩnh viễn**; mọi đánh giá mới sinh **result mới cho boundary mới**, không ghi đè và không hồi tố hợp thức hóa hay vô hiệu hóa quá khứ.
...
- **configuration/profile** khi capability phụ thuộc config.
