...
- **canonical authority designation version** cho *policy identity/version* (§10.4.3 mục 3);
- **canonical authority designation version** cho *runtime applicability* trong subject scope (§10.4.3 mục 4);
- **authoritative applicability/activation fact hoặc frontier** đã resolve tại evaluation boundary — bằng chứng policy đó thực sự **active trong scope** lúc đánh giá;
...
- **evaluator authorization/grant** — không chỉ một reference, mà đủ để chứng minh grant **đang có hiệu lực trong scope tại boundary**:
  - grant **identity + immutable version/content identity**;
  - **declared grant scope**;
  - **canonical grant-authority designation version** (cùng quy tắc exactly-one authority của §10.4.3, áp cho grant authority);
  - **authoritative grant activation/applicability fact hoặc frontier** đã resolve tại evaluation boundary;
  - bằng chứng grant **chưa bị revoke** và **bao phủ đủ**: policy version × subject scope × quyền phát hành eligibility result × boundary;
- **evaluation time/boundary**;
- **result** + **reason classification** (§10.4.2) + evidence references.

**`module identity ≠ authorization`, và `grant artifact tồn tại ≠ grant đang có hiệu lực`.** Pin identity/artifact chỉ trả lời *ai đã chạy phép đánh giá*; pin một grant reference chỉ trả lời *có tồn tại một grant nào đó*. Cả hai đều chưa trả lời *người đó có quyền, đúng phạm vi, tại thời điểm đó không*. Ví dụ hỏng: grant `G7` cấp cho evaluator `E` trong scope `Paper`, đã bị revoke tại T1; một result tại T2 pin đúng `G7` + version của nó nhưng cho scope `Live` vẫn trông hợp lệ nếu provenance chỉ đòi reference. Vì thế grant phải được pin **cùng cấp độ chứng cứ với policy** (§10.4.3): artifact **và** applicability fact/frontier, không chỉ artifact.

**Grant không resolve được authority/applicability · sai scope · chưa active · hoặc đã bị revoke tại boundary → Result INVALID**, không được đưa vào validated compatibility set. Việc grant bị revoke **sau đó** không sửa result lịch sử (§10.4.4); §10.5.1 mới là nơi quyết định result đó còn được dùng cho **current eligibility** hay không.

**Vì sao exact-pin policy artifact là chưa đủ:** "policy content là gì" và "policy đó có authority + đang active trong scope lúc đánh giá không" là **hai fact khác nhau**. §10.4.3 đã bắt designation phải version hóa; nếu result không pin designation đã dùng thì một thay đổi designation về sau sẽ **tái diễn giải** một result lịch sử:

```
T1: designation D1 → authority X trả lời "policy nào đang active"
     result ghi: policy = compat-v3 · boundary = T1 · eligible = true
sau: designation D2 → authority Y
     audit dùng D2 diễn giải lại applicability tại T1 → kết luận đổi hồi tố
```

**Designation thay đổi sau đó KHÔNG được làm thay đổi cách diễn giải một Compatibility Result lịch sử** — cùng nguyên tắc bất biến/không hồi tố ở §10.4.4 và §10.5.

**Result không resolve được evaluator identity · evaluator implementation/artifact · **grant identity/version, grant scope, grant-authority designation và grant applicability tại boundary** · compatibility policy/rule-set version · **authority designation version đã dùng** · evaluation scope/boundary thì KHÔNG hợp lệ và KHÔNG được đưa vào validated compatibility set** ([Chapter 9 §9.5](./09-plugin-model.md)). Schema cụ thể thuộc registry/Domain Contract; **provenance semantic là bắt buộc**.
...
### 10.5.1 Transition semantics — khi grant/applicability/designation đổi lúc subject đang active

§10.5 nói *khi nào* phải đánh giá, nhưng chưa nói **điều gì xảy ra với một subject đang active** khi nền tảng của kết luận cũ thay đổi. Không khóa thì mỗi implementation tự chọn: bên coi kết luận cũ vẫn có hiệu lực tới vô hạn, bên chặn ngay lập tức.

Ba loại transition thuộc phạm vi mục này: **(1)** evaluator grant thay đổi hoặc bị **revoke** · **(2)** policy applicability thay đổi (policy version khác trở thành active cho scope) · **(3)** authority designation thay đổi (§10.4.3).

**Bất biến chung cho cả ba — historical result KHÔNG đổi:** mọi Compatibility Result đã phát hành vẫn immutable và vẫn được diễn giải dưới đúng grant/policy/designation mà nó đã pin (§10.4.1, §10.4.4). Transition **không** hồi tố hợp thức hóa hay vô hiệu hóa quá khứ. Đây là điều kiện chung, không phải lựa chọn.

**Mô hình được khóa cho eligibility hiện hành: `revalidation-required`, KHÔNG phải `prospective-only`.** Lý do: eligibility là tuyên bố về **trạng thái hiện tại** ("subject này đang được phép"), không phải một sự kiện quá khứ. Nếu chọn prospective-only, một evaluator đã bị revoke quyền vẫn tiếp tục "bảo lãnh" cho subject đang chạy — mâu thuẫn trực tiếp với rule chống self-certification (§10.4.1) và với [I-6](./02-platform-invariants.md).

**Mỗi transition thuộc đúng MỘT trong hai lớp dưới đây** — không có lớp thứ ba, và một transition không được vừa "tiếp tục tới boundary kế tiếp" vừa "mất eligibility ngay":

| Lớp | Transition thuộc lớp này | Hiệu lực với subject đang active |
|---|---|---|
| **Immediate suspension** | evaluator grant bị **revoke** hoặc phát hiện evaluator vốn không có quyền · transition được policy phân loại **safety-critical** | Result cũ **hết hiệu lực làm current eligibility evidence NGAY** tại thời điểm transition. Subject **dừng sinh authoritative Decision**, chỉ resume sau khi có result hợp lệ mới. Fail-safe [I-6](./02-platform-invariants.md), **phạm vi nhỏ nhất nhưng đủ** |
| **Bounded revalidation** | policy applicability đổi **không** safety-critical · authority designation đổi (§10.4.3) | Result cũ **VẪN là current eligibility evidence** cho tới một **explicit, bounded revalidation deadline/boundary đã được pin**. Trong cửa sổ đó subject **ĐƯỢC PHÉP** tiếp tục sinh authoritative Decision dưới snapshot đã pin. **Quá deadline mà chưa có result hợp lệ mới → fail-safe I-6** |

**Hai rule khép kín, áp cho cả hai lớp:**
- **Mở activation MỚI luôn cần result hợp lệ dưới policy/designation đang áp dụng** — không lớp nào cho phép dùng result cũ để mở một activation boundary mới, vì validated compatibility set phải được validate tại chính boundary đó ([Chapter 9 §9.5](./09-plugin-model.md)). "Tiếp tục" ở lớp bounded chỉ có nghĩa **duy trì activation đang tồn tại**, không phải mở activation mới.
- **Không có deadline explicit + bounded + pin được thì KHÔNG thuộc lớp bounded** — mặc định rơi về **immediate suspension**. Nếu không, "bounded revalidation" trở thành cửa hoãn vô thời hạn.
...
**Chapter 10 sở hữu cấu trúc và semantic.** Capability Matrix **KHÔNG chỉ là danh sách execution mode** — nếu chỉ có execution mode thì nó không trả lời được subject có năng lực cụ thể nào, năng lực nào mới là declared, năng lực nào đã validated, và requirement nào đang thực sự được đáp ứng. Matrix có **hai chiều**:

```
Capability Matrix
├── capability assertions                     ← năng lực cụ thể
│   ├── required platform capability          (§10.2)
│   ├── provided contract capability          (§10.2)
│   └── business-capability implementation mapping (khi liên quan — Chapter 4)
└── execution-mode readiness                  ← kết luận dẫn xuất
    └── Backtest · Replay · Paper Trading · Live
```

**Execution-mode readiness là projection/kết luận được xây từ capability assertion**, không phải một cờ độc lập. **Danh sách execution mode canonical thuộc [Chapter 3 §3.1](./03-engineering-principles.md)** — Chapter 10 không tự định nghĩa danh sách khác và không tự thêm chiều mới bằng prose.

**Mỗi validated execution-mode readiness phải exact-pin hoặc resolve tới** capability requirement set · provider capability set · Compatibility Result/evidence tương ứng · policy/evaluator/boundary (§10.4.1). **Một entry `Live = validated` là KHÔNG hợp lệ nếu không resolve được tập capability requirement/provider evidence làm căn cứ** — nếu không, một provider bỏ đi một provided capability mà ô `Live = validated` vẫn đứng nguyên, và snapshot dù bất biến vẫn quá thô để giải thích readiness.
...
| Lớp | Ý nghĩa | Hệ quả |
|---|---|---|
| **Declared support** | Component **tuyên bố** năng lực nó cung cấp/đòi hỏi, và mode nó được thiết kế để hỗ trợ | **KHÔNG tự tạo eligibility**; là input để đánh giá, không phải kết luận |
...
**Hai lớp này không được gộp** trong cùng một ô "supports: yes" — áp cho **cả capability assertion lẫn execution-mode readiness** (§10.8).
