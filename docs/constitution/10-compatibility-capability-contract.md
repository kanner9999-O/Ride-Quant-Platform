...
- **evaluator authorization/grant reference + version** — bằng chứng evaluator **thực sự được quyền**: (a) dùng đúng policy version đó · (b) đánh giá đúng subject scope đó · (c) phát hành eligibility result tại đúng boundary đó;
- **evaluation time/boundary**;
- **result** + **reason classification** (§10.4.2) + evidence references.

**`module identity ≠ authorization`.** Pin identity và artifact chỉ trả lời *ai đã chạy phép đánh giá*, không trả lời *người đó có quyền đánh giá không*. §10.4.1 và §10.4.3 mục 7 đã khóa rằng quyền đánh giá là **grant** chứ không suy từ registry membership — nhưng nếu result không pin chính grant đó thì rule ấy không kiểm chứng được từ bản thân result, và một evaluator ngoài phạm vi được cấp quyền vẫn phát hành được result trông hợp lệ. Grant phải là **versioned reference** ([Chapter 9 §9.6](./09-plugin-model.md) — Grant layer, `granted ⊆ declared`), không phải một khẳng định trong prose của result.
...
**Result không resolve được evaluator identity · evaluator implementation/artifact · **evaluator authorization/grant version** · compatibility policy/rule-set version · **authority designation version đã dùng** · evaluation scope/boundary thì KHÔNG hợp lệ và KHÔNG được đưa vào validated compatibility set** ([Chapter 9 §9.5](./09-plugin-model.md)). Schema cụ thể thuộc registry/Domain Contract; **provenance semantic là bắt buộc**.
...
| Transition | Hiệu lực với subject đang active |
|---|---|
| **Evaluator grant bị revoke**, hoặc phát hiện evaluator vốn không có quyền tương ứng | **Hiệu lực ngay.** Result do evaluator đó phát hành **không còn dùng được làm eligibility evidence**; subject nào chỉ dựa vào result đó phải **revalidate**. Chờ revalidate → fail-safe theo I-6, **phạm vi nhỏ nhất nhưng đủ** |
| **Policy applicability đổi** (version khác active cho scope) | **Revalidation required.** Activation hiện hành tiếp tục dưới snapshot đã pin **cho tới activation boundary kế tiếp** ([Chapter 9 §9.5](./09-plugin-model.md)) — **trừ khi** transition được policy phân loại là **safety-critical**, khi đó hiệu lực ngay như dòng trên |
| **Authority designation đổi** (§10.4.3) | Diễn giải result lịch sử **giữ nguyên** dưới designation đã pin. Mọi đánh giá **mới** phải dùng designation mới; subject đang active phải revalidate **chậm nhất tại activation boundary kế tiếp** |
...
**Trong cửa sổ từ lúc transition có hiệu lực đến khi có result hợp lệ mới:** subject **không được coi là eligible** để mở **activation mới** hay để sinh authoritative Decision qua đường phụ thuộc result cũ. Đây là mặc định an toàn, không phải tùy chọn vận hành. Cơ chế thực thi (drain, grace window có bound, coordinator) thuộc **Phase 1** (§10.9); **semantic transition khóa ở đây**.
