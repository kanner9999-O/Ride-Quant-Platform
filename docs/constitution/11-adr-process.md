...
>
> **Governance migration (v2.3, historical — controlling from 2026-09-10T15:51+07:00 until the v2.4 boundary below):** §11.5/§11.9 dưới đây kích hoạt mô hình đã được Product Owner approve tại [ADR-042](../adr/ADR-042.md) (Approved) — cùng nội dung với [Chapter 0 §3 v1.3](./00-governance.md) (Locked, cùng activation boundary). Atomic Activation Boundary hoàn tất TẠI ĐÚNG activation commit này, đồng bộ CÙNG Chapter 0 §3, [Chapter 12](./12-approval-gates.md) (v1.7, Locked), ADR template, Global Execution Rules (v0.6, EFFECTIVE), Phase-3 rules (v0.3, EFFECTIVE), và MANIFEST — retire `ADR-031` Mode A/Mode B như mandatory approval-eligibility mechanism, thay bằng Review A mandatory + Risk Classification (R0/R1/R2) + optional advisory cross-check tại R2 (Product Owner chọn). `ADR-031` KHÔNG bị sửa (bất biến, Chapter 11 §11.3); định nghĩa của nó vẫn còn giá trị lịch sử; current lifecycle state của nó nay `Superseded` (bởi `ADR-042`), ghi tại MANIFEST theo I-12. Product Owner decision nguyên văn: "APPROVE ADR-042 v0.5 AND ITS ATOMIC GOVERNANCE ACTIVATION at reviewed semantic boundary 64fc05becc6ca4245229db79b3fb6c5d9187e622, based on current main e975d44f813b1ee91d2dbf3793376ca1827dc0f1," `2026-09-10T15:51+07:00`. Đoạn này giữ nguyên như bằng chứng lịch sử chính xác của migration đó; KHÔNG còn mô tả mô hình đang controlling kể từ boundary v2.4 dưới đây trở đi — current lifecycle/model state authoritative tại MANIFEST theo I-12.
>
> **Governance migration (v2.4, ACTIVE):** §11.5/§11.9 dưới đây kích hoạt mô hình đã được Product Owner approve tại [`ADR-045`](../adr/ADR-045.md) (Approved, `supersedes: [ADR-042]`) — cùng nội dung với [Chapter 0 §3 v1.5](./00-governance.md) (Locked, cùng activation boundary). Atomic Activation Boundary hoàn tất TẠI ĐÚNG activation commit này, đồng bộ CÙNG Chapter 0 §3, Global Execution Rules (v0.7, EFFECTIVE), và MANIFEST — `ADR-045`, self-contained for the R0/R1/R2 risk taxonomy (`X-MAJ-02`), is now the current Risk Classification definition source, replacing `ADR-042`. This activation changed ONLY that definition-source pointer in §11.5 and §11.9 below — narrow, mechanical wording-sync scope only: ADR approval remains Product-Owner-only (unchanged in substance); Review A remains mandatory (unchanged); the optional R2 cross-check semantics remain unchanged; the §11.9 validator still does NOT check cross-check existence/choice/evidence. This chapter does NOT govern Delegated Technical Resolution outcomes generally — it remains the ADR-process authority only; DTR's own eligibility/recording rules live entirely in `ADR-045`/`G-DELEGATE-001`/`G-DELEGATE-002`. `ADR-042` KHÔNG bị sửa (bất biến, Chapter 11 §11.3); định nghĩa của nó vẫn còn giá trị lịch sử; current lifecycle state của nó nay `Superseded` (bởi `ADR-045`), ghi tại MANIFEST theo I-12. Product Owner decision nguyên văn: "APPROVE ADR-045 v0.3 and its atomic activation. Accept Review A CLEAN — 0 Blocker / 0 Major / 1 Minor, Risk R2. Fold the non-semantic provenance-wording cleanup into activation. No additional cross-check required." — `2026-09-23T14:02+07:00`.
...
- Risk Classification bắt buộc, ngay sau Review A — đúng một trong R0/R1/R2 (định nghĩa đầy đủ: nguồn định nghĩa hiện tại LÀ [`ADR-045`](../adr/ADR-045.md) (Approved, tự chứa đầy đủ, KHÔNG đổi substance); [ADR-042](../adr/ADR-042.md) (Superseded) LÀ nguồn gốc lịch sử); R0/R1 mặc định `NO CROSS-CHECK`; R2: Review A recommend optional cross-check, KHÔNG BAO GIỜ tự động yêu cầu, Product Owner chọn `CROSS-CHECK` hoặc `PROCEED WITHOUT CROSS-CHECK`;
...
- Risk Classification present và resolve đúng một trong R0/R1/R2 (định nghĩa hiện tại tại [`ADR-045`](../adr/ADR-045.md) (Approved); [ADR-042](../adr/ADR-042.md) (Superseded) LÀ nguồn gốc lịch sử);
