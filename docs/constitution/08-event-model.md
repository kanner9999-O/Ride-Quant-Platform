...
- Version reference phải xác định duy nhất đúng nội dung registry — khuyến nghị kèm `registry_checksum` (có thể lưu ở run manifest thay vì lặp trên mọi event, miễn truy được tới immutable content).
...
- **Stream thuộc scope nhưng chưa có event visible**: phải biểu diễn tường minh bằng **`genesis_position` được định nghĩa trong đúng Stream Registry version mà cursor pin** (§8.3.1 — Registry sở hữu genesis position, KHÔNG phải Event Contract), **KHÔNG được để field vắng mặt** — vắng mặt là mơ hồ giữa "chưa có event" và "quên ghi".
