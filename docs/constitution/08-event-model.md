...
**Cursor validity:** `cursor.stream_registry_version` chỉ hợp lệ khi **(a)** nó là Genesis Registry, **hoặc (b)** `registry.effective_from.event_ref.sequence ≤ cursor.lifecycle_frontier.sequence` (cả hai reference nằm trên canonical lifecycle stream nên phép so có nghĩa — §8.3.5).

**Về version đã bị supersede:** cursor **được phép** tiếp tục pin version cũ cho historical replay và contract stability, miễn version đó **đã từng visible tại hoặc trước frontier**. KHÔNG bắt buộc pin "latest version" — bắt buộc như vậy sẽ khiến một registry transition hồi tố thay đổi Input Contract lịch sử.
...
Protocol cụ thể (ví dụ `stop writer → confirm last sequence → append retirement event`, hay atomic coordinator protocol khác) do ADR-009 chọn; Constitution khóa **outcome**.
...
**Validity invariant khi `position.kind: event`** (tương đương mức chặt của `stream_positions`): `lifecycle_frontier.(stream_id, sequence)` phải resolve tới một **committed authoritative event record trên canonical lifecycle stream**, và event đó phải thỏa:
...
| **Registry → Lifecycle** | `registry.effective_from.sequence ≤ cursor.lifecycle_frontier.sequence` (§8.3.1) |
