...
> **Trạng thái:** `Locked`. Product Owner đã **Approve and Lock** Chapter 13 **v1.8** (`2026-09-23T09:34:00+07:00`, `FE-EVID03-COND1-PROTOCOL-ACTIVATION-001`), superseding v1.7 as the chapter's controlling version. Theo [Chapter 12 §12.3](./12-approval-gates.md), chương này từ nay là **binding authoritative Quality Gates contract** — đúng contract mà [Chapter 12 §12.2(5)](./12-approval-gates.md) yêu cầu cho applicable quality gates. Quality Gate vẫn khác Product Owner Approval Gate (§13.1) — trạng thái Locked không đổi phân biệt đó.
>
> **v1.8 ACTIVATED (2026-09-23) — `Locked`, controlling.** Authored per Chapter 0 §5.1 (a Locked living document cannot be edited in place at the same version — a new candidate version must be authored and pass its own fresh approval gate). Adds new §13.8.1 ("Bounded-measurement reproducibility for `UNSTABLE_TIMEOUT_TRIAGE` populations"). Reviewed at boundary `704e492f83af1d463117f002c9f319ebcadd8099`: Review A (ChatGPT, `AI Technical Architect`) `CLEAN — 0 Blocker / 0 Major / 2 Minor`; optional Product-Owner-selected R2 advisory cross-check (Claude, `AI Technical Architect`) `0 Blocker / 0 Major / 5 Minor` — all seven combined Minors independently re-verified and accepted by ChatGPT as non-blocking, Risk `R2`. Product Owner approved the reviewed v0.5/v1.8 semantic bundle and activated it atomically together with `docs/adr/ADR-044.md` v0.5 (`Approved`). **`CONSOLIDATION` (root-cause pass, `P3-CORRECTION-CHAIN-001`, following three narrow correction rounds that did not converge):** §13.8.1 is the **sole normative source** for this rule — self-contained, requiring no reference to any ADR to understand or apply correctly. `docs/adr/ADR-044.md` v0.5 (`Approved`) carries the architecture rationale, alternatives, and consequences only, with no second executable copy of the gate semantics; a clause-trace table there maps every ADR guarantee to its exact normative clause here. If ADR prose and this section ever disagree, **this section controls Quality-Gate semantics** (§13.13). This consolidation restored the per-mutant/gate-level reproducibility reconciliation that a prior correction round had inadvertently dropped, clarified Testing Convention item 8's evidence-layer precedence without ambiguous wording, and made protocol provenance a structural (property-based) requirement rather than a reference to an unpinned "presently-governed" protocol. Two residual Minor findings from the R2 cross-check — `X3-MIN-04` (undefined "governing evaluation contract" term) and `X3-MIN-05` (overlapping qualification wording), both in §13.8.1 — were explicitly accepted by the Product Owner as **non-blocking residual findings** and deliberately **not remediated** by this activation; §13.8.1's normative text was not touched for either. **`v1.7` is now this chapter's historical predecessor, no longer controlling** — see MANIFEST for the current authoritative pointer (I-12). No section of this chapter other than this banner, frontmatter, and §13.8.1 (activated at this transaction's predecessor boundary) is touched by this activation; §13.8's own existing fail-closed text (above §13.8.1) is byte-unchanged. **This activation does not re-evaluate, apply, or change any current Quality-Gate result** — Condition 1 for `P3-FEATURE-QG-EVID-03` remains `STOPPED / UNRESOLVED` until a separate, subsequent, bounded application transaction formally applies this now-effective rule.
