...
| ADR-014 | **Draft** (chưa Product Owner approve) | 2026-07-29 | Narrow amendment ADR-003: phân biệt tường minh Feature computation fan-in (Feature Engine sở hữu) với Context snapshot aggregation (Context sở hữu) — giữ nguyên toàn bộ Raw Regime/Structure độc lập của ADR-003. `supersedes: [ADR-003]` trong frontmatter là forward relation ĐỀ XUẤT, CHƯA có hiệu lực — ADR-003 vẫn `Approved`, bất biến, cho tới khi ADR-014 chính thức Approved (Chapter 11 §11.8 atomic supersede). Chưa có review nào (reviewers: [], tối thiểu hai independent review bắt buộc trước Product Owner quyết định, Chapter 11 §11.5). Package 0.2-B4 blocked khỏi `Consolidated Stable` cho tới khi ADR-014 Approved. |
