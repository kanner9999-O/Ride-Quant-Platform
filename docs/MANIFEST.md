...
| File | Status |
|---|---|
| domain/ | Phase 0.2 started (Draft) — **Package 0.2-A: `Consolidated Stable`** (không đổi) — `context-map.yaml` **v0.6** Draft (bump: `raw-regime-analysis.owned_contracts` chuyển forward-declared → authored, không đổi relationship semantics) + `candle.md` **v0.4** Draft (không đổi). [ADR-012](adr/ADR-012.md) v0.3 và [ADR-013](adr/ADR-013.md) v0.3 vẫn **`Approved`** (không đổi). **Package 0.2-B1: `Consolidated Stable`** (không đổi) — `swing.md` **v0.2** Draft (blob `5bbe666ff404209876a721b1e01cb9ac62011062`) + `structure.md` **v0.4** Draft (blob `78964dfb6852bbac3fa1e034d64b4fc8031c3fef`), reviewed HEAD `d6545de3fcc767e03f74fd0712ada792372d1c33`. **Package 0.2-B2: `Consolidated Stable`** — `regime.md` **v0.2** Draft (blob `edd1584377f1db84269e7b1dfdd4926d0ce01c70`), reviewed HEAD `78479ab088b1a32c580c9a729a53333896b952b3`. ChatGPT Review A: Clean — Blocker 0, Major 0, Minor 0. Independent Review B narrow delta: `IRB-B2-MAJ-01`/`IRB-B2-MAJ-02`/`IRB-B2-MIN-03` resolved — Blocker 0, Major 0, Minor 0, Suggestion 0; `regime.md` v0.2 Clean; Package integration Clean. Package lifecycle state — KHÔNG phải document approval; `regime.md` `status: Draft`, `approved_by: null` không đổi. Dependency `candle.md` v0.4/`swing.md` v0.2/`structure.md` v0.4 không đổi trong suốt B2. **Package 0.2-B3: `Consolidated Stable`** — `feature.md` **v0.2** Draft (blob `2262adf9253ea20c8d817d1066f50c4353d2d35d`), reviewed HEAD `ed8813030203cd9e5f779f54be752a3e94c4f68b`. Scope: 3 founding feature type (`volatility_metric`, `directional_persistence_metric`, `distance_to_last_confirmed_swing`), không mở rộng framework, không Feature-to-Feature dependency, không ADR mới. ChatGPT Review A final delta: `RA-B3-MAJ-01` resolved — Blocker 0, Major 0, Minor 0, Suggestion 0. Independent Review B narrow delta: `IRB-B3-MAJ-01` resolved — Blocker 0, Major 0, Minor 0, Suggestion 0; `feature.md` v0.2 Clean; Package integration Clean; Consolidation readiness: Ready. Package lifecycle state — KHÔNG phải document approval; `feature.md` `status: Draft`, `approved_by: null`, `approved_at: null` không đổi. Dependency `candle.md` v0.4/`swing.md` v0.2/`structure.md` v0.4/`regime.md` v0.2 không đổi trong suốt B3. **Package 0.2-B4: narrow architecture revision** (Product Owner authorized: "Authorize narrow amendment of ADR-003 to distinguish Feature computation fan-in from Context snapshot aggregation.") — `context.md` **v0.1 → v0.2** Draft (blob mới, xem CHANGELOG), xử lý `RA-B4-MAJ-01`/`IRB-B4-MAJ-01` (cùng một algorithmic defect — Eligible Upstream Fact selection §8 tham chiếu ngược một bước sau nó, sửa thành hai phase tách bạch), `IRB-B4-MAJ-02` (`missing_input_policy` từ chuỗi tự do → enum đóng một giá trị `NO_SNAPSHOT_WHEN_ANY_REQUIRED_ROLE_MISSING_OR_PENDING` — resolved technically), `IRB-B4-MAJ-03` (xung đột ADR-003 — xem `adr/ADR-014.md` dưới đây — addressed technically qua ADR-014 Draft, governance-open cho tới khi Approved). **Narrow traceability correction (`RA-B4-MIN-02`, transaction sau):** commit gốc (`f1ea03b`) từng gán ngược hai ID này — đã sửa, xem CHANGELOG. `context-map.yaml` **v0.8 → v0.9** Draft (bump: 10 relationship B4 hiện có bổ sung field `purpose` phân biệt tường minh "authoritative as-of snapshot aggregation, không phải Feature computation" — KHÔNG đổi provider/consumer/contract_id nào; preserve nguyên vẹn 10 relationship). **ADR-014 author cùng transaction, sau đó narrow review correction v0.1 → v0.2, sau đó Product Owner approve (2026-07-30)** — đóng `IRB-ADR014-MAJ-01` (Definition-pinned direct fan-in rule, ngăn layer-level broad permission), `IRB-ADR014-MAJ-02` (Context prohibition list đầy đủ — Risk/Account/Position/Execution/order authority, cấm rename-conclusion-as-context-value), `IRB-ADR014-MIN-01` (sửa stale finding reference trong `context.md`, traceability-only), `IRB-ADR014-MIN-02` (ghi nhận 6 risk + mitigation); ChatGPT + Claude narrow delta review Clean 0/0/0/0 mỗi bên; Product Owner "Approve ADR-014." + accept 6 residual risk có điều kiện mitigation; xem dòng ADR-014 phía trên. `ADR-003.md` **giữ nguyên byte-for-byte** (Chapter 11 §11.3), embedded `status: Approved` không đổi — **current authoritative lifecycle state chuyển `Superseded` bởi ADR-014** (dòng ADR-003 phía trên), atomic cùng ADR-014 `Approved` (Chapter 11 §11.8). **`IRB-B4-MAJ-03` (ADR-003 fan-in conflict) NAY governance-resolved** — architecture blocker của Package 0.2-B4 đã gỡ. **Package 0.2-B4 VẪN `Draft`, KHÔNG tự động `Consolidated Stable`/Approved/Locked** — technical review clean (20 attack scenario B4 v0.2, 16 attack scenario ADR-014 v0.2) và architecture blocker cleared, nhưng B4 còn cần transaction package delta review/consolidation riêng của chính nó trước khi đạt `Consolidated Stable`. **Package 0.2-C: gate open, vẫn chưa có artifact nào được author.** OQ-002/OQ-003 vẫn `Open`. Phase 0.2 vẫn active và chưa hoàn tất. Không authorize Live ở bất kỳ hình thức nào. |
