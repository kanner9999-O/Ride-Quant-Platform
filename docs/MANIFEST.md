...
| File | Status |
|---|---|
| domain/ | Phase 0.2 started (Draft) — **Package 0.2-A: `Consolidated Stable`** (không đổi) — `context-map.yaml` **v0.6** Draft (bump: `raw-regime-analysis.owned_contracts` chuyển forward-declared → authored, không đổi relationship semantics) + `candle.md` **v0.4** Draft (không đổi). [ADR-012](adr/ADR-012.md) v0.3 và [ADR-013](adr/ADR-013.md) v0.3 vẫn **`Approved`** (không đổi). **Package 0.2-B1: `Consolidated Stable`** (không đổi) — `swing.md` **v0.2** Draft (blob `5bbe666ff404209876a721b1e01cb9ac62011062`) + `structure.md` **v0.4** Draft (blob `78964dfb6852bbac3fa1e034d64b4fc8031c3fef`), reviewed HEAD `d6545de3fcc767e03f74fd0712ada792372d1c33`. **Package 0.2-B2: `Consolidated Stable`** — `regime.md` **v0.2** Draft (blob `edd1584377f1db84269e7b1dfdd4926d0ce01c70`), reviewed HEAD `78479ab088b1a32c580c9a729a53333896b952b3`. ChatGPT Review A: Clean — Blocker 0, Major 0, Minor 0. Independent Review B narrow delta: `IRB-B2-MAJ-01`/`IRB-B2-MAJ-02`/`IRB-B2-MIN-03` resolved — Blocker 0, Major 0, Minor 0, Suggestion 0; `regime.md` v0.2 Clean; Package integration Clean. Package lifecycle state — KHÔNG phải document approval; `regime.md` `status: Draft`, `approved_by: null` không đổi. Dependency `candle.md` v0.4/`swing.md` v0.2/`structure.md` v0.4 không đổi trong suốt B2. **Package 0.2-B3: đã bắt đầu** (Product Owner authorized: "Authorize Package 0.2-B3 minimal Feature scope.") — `feature.md` **v0.1** Draft authored, `capability_id: feature-engineering` / `domain_context_id: feature-engineering` (đã đăng ký sẵn, không tạo context/capability mới). `context-map.yaml` **v0.7** Draft (bump: `feature-engineering.owned_contracts` forward-declared → authored, cộng 6 relationship mới cho candle-closed/candle-corrected/swing-confirmed/swing-invalidated/regime-classified/regime-fact-invalidated → feature-engineering — không đổi relationship hiện có). Scope: 3 founding feature type (`volatility_metric`, `directional_persistence_metric`, `distance_to_last_confirmed_swing`), không mở rộng framework, không Feature-to-Feature dependency, không ADR mới. Author self-review v0.1 hoàn tất. **Narrow revision v0.1 → v0.2:** `feature.md` **v0.2** Draft (blob mới, xem CHANGELOG) — xử lý `RA-B3-MAJ-01`/`IRB-B3-MAJ-01` (effective-time look-ahead trong eligible-Swing selection cho `distance_to_last_confirmed_swing`): thêm `eligible_swing_effective_cutoff_policy: REFERENCE_CANDLE_WINDOW_END_EXCLUSIVE` (§6), viết lại §9a thành ordered filter pipeline 5 bước (effective-time cutoff là filter độc lập, chạy TRƯỚC total order 8 tiêu chí), strengthen §12/§13. Author self-review revision hoàn tất (16 attack scenario). `context-map.yaml` giữ nguyên **v0.7**, byte-for-byte không đổi qua revision này. **ChatGPT Review A và Independent Review B CHƯA diễn ra** (sẽ chạy trên v0.2) — Package 0.2-B3 **chưa `Consolidated Stable`**. **Package 0.2-B4** (`context.md`): chưa bắt đầu. **Package 0.2-C: gate open, vẫn chưa có artifact nào được author.** OQ-002/OQ-003 vẫn `Open`. Phase 0.2 vẫn active và chưa hoàn tất. Không authorize Live ở bất kỳ hình thức nào. |
