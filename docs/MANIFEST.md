...
| ADR-014 | **Draft v0.2** (chưa Product Owner approve) | 2026-07-30 | Narrow amendment ADR-003: phân biệt tường minh Feature computation fan-in (Feature Engine sở hữu) với Context snapshot aggregation (Context sở hữu) — giữ nguyên toàn bộ Raw Regime/Structure độc lập của ADR-003. v0.2 (narrow review correction, Independent Review B trên v0.1): thêm quy tắc Definition-pinned direct fan-in (đóng `IRB-ADR014-MAJ-01`); mở rộng Context prohibition list đầy đủ — Risk/Account/Position/Execution/order authority (đóng `IRB-ADR014-MAJ-02`); sửa stale finding reference trong `context.md` (đóng `IRB-ADR014-MIN-01`); ghi nhận 6 risk + mitigation, không ngụ ý Product Owner accept (đóng `IRB-ADR014-MIN-02`). `supersedes: [ADR-003]` trong frontmatter là forward relation ĐỀ XUẤT, CHƯA có hiệu lực — ADR-003 vẫn `Approved`, bất biến, cho tới khi ADR-014 chính thức Approved (Chapter 11 §11.8 atomic supersede). `reviewers: []` giữ nguyên — findings trên đến từ Independent Review B (đã ghi nhận trong ADR concerns/risks), nhưng một tập hai-reviewer hợp lệ theo Chapter 11 §11.5 CHƯA xác lập cho chính ADR-014; bản v0.2 CHƯA qua delta review. Package 0.2-B4 blocked khỏi `Consolidated Stable` cho tới khi ADR-014 Approved. |
