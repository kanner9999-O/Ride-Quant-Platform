...
| File | Status |
|---|---|
| domain/ | Phase 0.2 started (Draft) — **Package 0.2-A: `Consolidated Stable`** (không đổi) — `context-map.yaml` **v0.6** Draft (bump: `raw-regime-analysis.owned_contracts` chuyển forward-declared → authored, không đổi relationship semantics) + `candle.md` **v0.4** Draft (không đổi). [ADR-012](adr/ADR-012.md) v0.3 và [ADR-013](adr/ADR-013.md) v0.3 vẫn **`Approved`** (không đổi). **Package 0.2-B1: `Consolidated Stable`** (không đổi) — `swing.md` **v0.2** Draft (blob `5bbe666ff404209876a721b1e01cb9ac62011062`) + `structure.md` **v0.4** Draft (blob `78964dfb6852bbac3fa1e034d64b4fc8031c3fef`), reviewed HEAD `d6545de3fcc767e03f74fd0712ada792372d1c33`. **Package 0.2-B2: active, revision required → revised** — `regime.md` **v0.2** Draft (blob mới, xem CHANGELOG) — đóng `RA-B2-MIN-01`/`IRB-B2-MIN-03` (Current View no-row semantics, bỏ `UNAVAILABLE`), `IRB-B2-MAJ-01` (canonical Candle evidence normalization — tập toán học, 6-tiêu-chí lexicographic order), `IRB-B2-MAJ-02` (invalidation envelope binding — `subject_ref`/`effective_time` kế thừa từ fact bị invalidate). Scope không đổi: Volatility + Directional Persistence, continuous 5-field subject identity, không ADR mới. `context-map.yaml`, `swing.md`, `structure.md` **không đổi** ở revision này. Author self-review hoàn tất; **ChatGPT Review A và Independent Review B re-review CHƯA diễn ra** — Package 0.2-B2 **chưa `Consolidated Stable`**. **Package 0.2-B3/B4** (`feature.md`, `context.md`): chưa bắt đầu. **Package 0.2-C: gate open, vẫn chưa có artifact nào được author.** OQ-002/OQ-003 vẫn `Open`. Phase 0.2 vẫn active và chưa hoàn tất. Không authorize Live ở bất kỳ hình thức nào. |
