...
| Next architecture prerequisite | `docs/adr/ADR-046.md` (`Context Computation Cursor and Temporal Eligible-Upstream Supersession`) — `v0.5`, `status: Draft`, `NOT APPROVED`, Risk `R2`, ADR Scope `ADR_REQUIRED`. Round-1 Review A on v0.1 returned `REVISION_REQUIRED — 0/4/2` (remediated by `ADR-046-CORR-001`); round-2 Review A on v0.2 returned `REVISION_REQUIRED — 0/3/0` (remediated by `ADR-046-CORR-002`); round-3 Review A on v0.3 closed all nine prior findings and returned `REVISION_REQUIRED — 0/1/0` on `MAJ-R3-01` (remediated by `ADR-046-CORR-003`); round-4 Review A on v0.4 **closed all ten prior findings** and returned `REVISION_REQUIRED — 0/2/0` on `MAJ-R4-01`/`MAJ-R4-02` (remediated by `ADR-046-CORR-004`) — awaiting fresh Review A re-review. Establishes a durable `computation_cursor` (canonical Chapter 8 §8.5 Replay Cursor) for `MarketContextSnapshot`/`MarketContextFactInvalidated`, its `Cursor → Context projection record` anti-look-ahead relation, a bounded `COVERS_CONTEXT` Context-scoped knowledge-non-regression relation (partial, reasoning-only, no new payload field, recorded-time non-regression leg included) applied as a universal precondition to EVERY current-lineage invalidation (Decision item 3a) — not only temporal supersession, and explicit invalidation/replacement-cursor Case A/B rules (Case B: fresh non-counterfactual re-evaluation, 10 sub-rules, `R_replacement COVERS_CONTEXT R_later`; Case A legal only because item 3a's precondition already holds at invalidation; resulting coverage chain `K_context(R_original) ⊆ K_context(R_later) ⊆ K_context(R_replacement)`, transitive set inclusion, never a global order). Runtime/Input Contract/publishing layer work is blocked pending this decision's current-model review chain: fresh Review A → Product Owner decision (Risk fixed `R2`; an optional advisory cross-check may be Product-Owner-selected, never an approval prerequisite). |
...
Current primary WP:       (none currently assigned) -- context-aggregator
                           ...
                           `feature-context-architecture.md`/any
                           existing ADR (including ADR-009) NOT
                           touched. Next governed action: fresh
                           ChatGPT Review A re-review of `ADR-046`
                           v0.5 corrected Draft candidate.
                           Prior Condition-1
