...
| Next architecture prerequisite | `docs/adr/ADR-046.md` (`Context Computation Cursor and Temporal Eligible-Upstream Supersession`) — `v0.2`, `status: Draft`, `NOT APPROVED`, Risk `R2`, ADR Scope `ADR_REQUIRED`. Round-1 ChatGPT Review A of v0.1 returned `REVISION_REQUIRED — 0 Blocker / 4 Major / 2 Minor`; all six findings addressed/remediated by `ADR-046-CORR-001` (none self-closed) — awaiting fresh Review A re-review. Establishes a durable `computation_cursor` (canonical Chapter 8 §8.5 Replay Cursor) for `MarketContextSnapshot`/`MarketContextFactInvalidated`, a bounded temporal eligible-winner-supersession invalidation rule (role-resolution-delta framing, per-role cause-ref set), and explicit invalidation/replacement-cursor Case A/B rules. Runtime/Input Contract/publishing layer work is blocked pending this decision's review chain (ChatGPT Review A re-review → Review B → Product Owner). |
...
Current milestone:        M3 — Context Projection / context-aggregator
                           ...
                           addressed/remediated, none self-closed) as
                           the next architecture prerequisite, pending
                           fresh Review A re-review; M3 NOT DONE -- runtime/Input
                           Contract/Event Contract/publishing/Strategy
                           integration still deliberately
                           unimplemented, now blocked pending ADR-046's
                           review chain). M1
...
Current primary WP:       (none currently assigned) -- context-aggregator
                           ...
                           `ADR-046` v0.2 corrected Draft candidate.
                           Prior Condition-1
                           ...
                           Majors/2 Minors (`ADR-046-CORR-001`, this
                           transaction) -- M1 DONE, M2 BLOCKED (parallel
                           lane), M3 ACTIVE (Context Projection /
                           context-aggregator, deterministic core
                           Review-A-validated CLEAN, ADR-046 v0.2
