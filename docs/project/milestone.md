...
| Next architecture prerequisite | `docs/adr/ADR-046.md` (`Context Computation Cursor and Temporal Eligible-Upstream Supersession`) — `v0.1`, `status: Draft`, `NOT APPROVED`, Risk `R2`, ADR Scope `ADR_REQUIRED`. Establishes a durable `computation_cursor` (canonical Chapter 8 §8.5 Replay Cursor) for `MarketContextSnapshot`/`MarketContextFactInvalidated` and a bounded temporal eligible-winner-supersession invalidation rule. Runtime/Input Contract/publishing layer work is blocked pending this decision's review chain (ChatGPT Review A → Review B → Product Owner). |
...
Current milestone:        M3 — Context Projection / context-aggregator
                           ...
                           self-reviewed) as the next architecture
                           prerequisite; M3 NOT DONE -- runtime/Input
...
Current primary WP:       (none currently assigned) -- context-aggregator
                           ...
                           roles. Next governed action: fresh ChatGPT
                           Review A of the ADR-046 Draft candidate.
                           Prior Condition-1
                           ...
                           (`CONTEXT-CORE-REVIEW-A-DTR-001` +
                           `ADR-046-AUTHOR-001`, this transaction) --
                           M1 DONE, M2 BLOCKED (parallel lane), M3
                           ACTIVE (Context Projection /
                           context-aggregator, deterministic core
                           Review-A-validated CLEAN, next architecture
                           prerequisite ADR-046, runtime/publishing/
                           Strategy integration not started), M4
                           QUEUED
