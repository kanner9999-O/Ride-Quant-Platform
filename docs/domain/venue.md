...
2. **`VenueRegistered`** (`kind: event`) — đăng ký lần đầu.
3. **`VenueMetadataRevised`** (`kind: event`) — forward-looking metadata change (timezone/calendar/session policy, precision/increment default).
4. **`VenueOperationalStatusChanged`** (`kind: event`) — operational status transition.

Cộng **`VenueFactInvalidated`** (correction lineage) và một **read model tùy chọn** (`VenueCurrentView`).
...
**Áp dụng ngay từ v0.1 mọi bài học đã trả giá xuyên suốt Package 0.2-B/instrument.md:** envelope binding cho `VenueFactInvalidated`; tách bạch forward-looking revision khỏi correction (đúng `instrument.md` §4/§16); no-row Current View semantics; canonical policy identifier khai báo ĐÚNG MỘT NƠI; opaque identity — không parse `venue_id` (Chapter 6 §6.8).
...
invariants:
  - "venue_id resolve deterministic từ scope identity bất biến (venue_type, jurisdiction_ref khi có) — cùng scope luôn cho cùng venue_id; khác bất kỳ field scope nào cho venue_id KHÁC. venue_id bất biến, KHÔNG tái sử dụng cho một subject khác (Chapter 6 §6.1)."
  ...
  - "venue_type bất biến sau khi subject được đăng ký lần đầu — đổi venue_type là tạo một Venue KHÁC (venue_id khác)."
  - "Logical Venue identity KHÔNG được mang production endpoint, sandbox endpoint, API credential, adapter instance, hay deployment environment (§9 — những khái niệm đó thuộc runtime/architecture, Phase 1, không phải Domain Model)."
schema:
  venue_id: {type: string, required: true, description: "opaque, stable — xem invariants"}
  venue_type: {type: enum, values: [CENTRALIZED_EXCHANGE, DECENTRALIZED_EXCHANGE, BROKER], required: true, description: "đóng ở v0.1, đủ tối thiểu để không hardcode crypto-CEX-only; mở rộng thêm giá trị là thay đổi Domain Contract tường minh"}
...
Mọi cardinality khác (`event_id`, `recorded_time`, `causation_refs`, `effective_time`, `market_time: PROHIBITED`, `source_identity`, `stream_ref`/`producer_ref`) — áp dụng nguyên văn theo `instrument.md` §2, không định nghĩa lại.
...
domain_context_id: instrument-venue-reference
description: >
  Fact AUTHORITATIVE, DUY NHẤT cho việc đăng ký lần đầu một Venue — thiết lập scope identity bất
  biến (venue_type, jurisdiction_ref khi có) VÀ mutable reference metadata ban đầu (timezone,
  default session/calendar policy, default precision policy).
invariants:
  - "Đúng MỘT VenueRegistered cho mỗi venue_id."
...
domain_context_id: instrument-venue-reference
description: >
  Fact AUTHORITATIVE cho một thay đổi metadata mô tả của Venue — ví dụ timezone/calendar policy
  đổi (đóng "venue timezone/calendar change"), default precision policy đổi. **Forward-looking
  theo mặc định** — cùng nguyên tắc `instrument.md` §4: fact liền trước vẫn hợp lệ cho window
  lịch sử của nó. Correction dùng VenueFactInvalidated (§6).
invariants:
  ...
  - "Ít nhất một field mutable PHẢI thay đổi so với fact liền trước."
payload:
  venue_id: {type: string, required: true}
  display_name: {type: string, required: false}
...
domain_context_id: instrument-venue-reference
description: >
  Phủ định MỘT VenueMetadataRevised hoặc VenueOperationalStatusChanged lịch sử cụ thể ĐÃ SAI —
  cùng nguyên tắc `instrument.md` §6. VenueRegistered KHÔNG BAO GIỜ là target hợp lệ.
invariants:
  ...
  - "invalidated_fact_ref PHẢI trỏ một VenueMetadataRevised hoặc VenueOperationalStatusChanged, CHƯA từng nhận invalidation khác."
...
domain_context_id: instrument-venue-reference
description: >
  Projection tiện dụng: metadata/status "hiện tại" của một Venue, rebuild được từ §3–§6. KHÔNG
  authoritative — mọi input cho Domain Contract khác PHẢI dùng authoritative event stream
  (`ref: venue`), KHÔNG BAO GIỜ dùng view này (I-12, Chapter 7 §7.4).
invariants:
...
schema:
  venue_id: {type: string, required: true}
  scope: {venue_type: string, jurisdiction_ref: string, required: true}
...
**Deferred tường minh, không author ở C1:** cơ chế resolve cụ thể `timezone_ref`/`default_session_calendar_ref`/`default_precision_policy_ref` (calendar/timezone service, Phase 1); production/sandbox endpoint, adapter instance, credential (Phase 1 Engineering/Plugin Model, §10); `stream_ref`/`producer_ref` (Phase 1); Account/Strategy/Decision/Risk/Order/Fill/Position/Execution (Package 0.2-C2–C7, chưa authorize).
