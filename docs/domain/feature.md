...
**v0.5 xử lý `P3-FEATURE-DC-A-MAJ-01`:** v0.4 đúng khi đổi §9a bước 2 sang §12's full three-leg computation-cursor visibility predicate, nhưng bước 4 ("visible tại R (swing.md §1a)") và bước 5 ("visible tại R ... (swing.md §5)") vẫn dùng cụm "visible tại R" mà không tường minh nói rõ đó là §12's predicate — trong khi `swing.md` §7's own replay-visibility rule (mà §1a/§5 gián tiếp dựa vào) chỉ là recorded-time-only (`event.recorded_time <= T`), một quy tắc KHÁC và YẾU HƠN §12's predicate kể từ ADR-035. Pipeline vì vậy có nguy cơ mang hai nghĩa "visible" khác nhau trong cùng một chuỗi 5 bước. v0.5 sửa: bước 4/bước 5 tường minh nói "full-cursor-visible tại R theo §12"; thêm một quy ước DUY NHẤT ngay sau pipeline khẳng định MỌI cụm "visible tại R" trong §9a (Bước 2/4/5, và mọi tham chiếu ngắn gọn khác) đều nghĩa là §12's predicate, KHÔNG PHẢI `swing.md` §7's công thức recorded-time-only — `swing.md` VẪN là authority DUY NHẤT cho revision lineage/matching (§1a) và invalidation semantics (§5), CHỈ KHÔNG PHẢI authority cho chính computation-cursor visibility rule. Không đổi §12's predicate, bước 3's effective-time cutoff, 8-tiêu-chí total order, ADR-034's bốn điều kiện supersession, causation binding, mutual exclusion, hay fail-closed artifact-resolvability boundary — tất cả giữ nguyên byte-tương đương.

**v0.6 implement [ADR-037](../adr/ADR-037.md) (Approved) + [ADR-038](../adr/ADR-038.md) (Approved) — persisted dependency content-identity evidence, explicit backward-only compatibility commitment, BREAKING classification, and a fail-closed version-identity blocker:** `FeatureComputed` (§3) và `FeatureFactInvalidated` (§4) mỗi cái bổ sung một payload field bắt buộc mới, `computation_dependency_content_evidence` — chứa đúng hai giá trị `input_contract_content_id`/`stream_registry_content_id`, sao chép nguyên văn từ CÙNG một `VerifiedInputContractAuthority` instance mà computation engine đã resolve/cache cho CHÍNH fact đó (ADR-037), tương ứng quan hệ CHÍNH XÁC với `computation_cursor.input_contract_ref`/`computation_cursor.stream_registry_version` của ĐÚNG cùng fact — KHÔNG PHẢI một replay-cursor/ordering concept, KHÔNG sửa canonical [Chapter 8](../constitution/08-event-model.md) §8.5 Replay Cursor. Mỗi fact — gốc, replacement, hay invalidation — pin evidence ĐỘC LẬP của chính nó, KHÔNG kế thừa evidence của fact trước đó, cùng discipline `computation_cursor` đã có từ v0.4. `feature-computed`/`feature-fact-invalidated` nay tuyên bố tường minh `compatibility_commitment: backward_only` (ADR-038, Approved) — [`10-compatibility-capability-contract.md`](../constitution/10-compatibility-capability-contract.md) §10.3.1: consumer CŨ vẫn đọc/validate đúng dữ liệu MỚI; forward compatibility KHÔNG cam kết.

Áp dụng trực tiếp §10.3.1's own Locked, format-independent minimal classification rule (*"thêm element required không có fallback → breaking"*) cho `computation_dependency_content_evidence`: delta này được phân loại **BREAKING** — kết luận này KHÔNG chờ, và KHÔNG cần, một reader/format-specific policy nào tồn tại trước. `ADR-038`'s own Compatibility/Consequences prose, khi áp cho MỘT delta cụ thể theo cách gợi ý classification bị chặn cho tới khi policy đó tồn tại, là authority THẤP HƠN §10.3.1's own direct Locked rule tại đây (Authority hierarchy: Constitution/Locked governance > Approved ADR — `docs/governance/execution-rules.md` `G-AUTH-003`, "Authority cao hơn LUÔN thắng"). Xung đột này được resolve TẠI ĐÂY, KHÔNG sửa/supersede `ADR-038` — `ADR-038` VẪN `Approved`, bất biến byte-for-byte, và quyết định `backward-only` của chính nó KHÔNG đổi; chỉ phần diễn giải classification-blocking khi áp cho delta cụ thể này được ghi đè bởi rule Locked cao hơn. Breaking classification, theo Chapter 10 §10.3's own version-axis discipline, đòi hỏi một Feature Output Event Contract identity MỚI (`event_contract_ref.contract_version`, Chapter 8 §8.2.5) khi field này thực sự được implement/emit — KHÔNG BAO GIỜ tái sử dụng identity hiện có cho một shape đã breaking.

**Fail-closed, KHÔNG bịa đặt một predecessor hay SemVer bump giả:** không tồn tại một baseline `contract_version`/`schema_version` PIN được, authoritative nào cho `feature-computed`/`feature-fact-invalidated` ở BẤT KỲ artifact nào trong repository này hiện tại — §2 dưới đây chỉ khai `event_contract_ref`/`schema_version` là required field, KHÔNG pin giá trị; [`context-map.yaml`](./context-map.yaml) chỉ pin `contract_id`, KHÔNG pin `contract_version`; test-only fixture constant KHÔNG PHẢI authority, đúng `resolve_output_contract_refs`'s own discipline (feature-engine KHÔNG BAO GIỜ tự phát minh version này, `P3-FEATURE-A-MAJ-02`) — cùng trạng thái deferral đã dùng xuyên suốt cho Stream Registry/Feature-scoped Input Contract artifact (Phase 1, chưa author). v0.6 KHÔNG mint một `contract_version` cụ thể nào. Minting identity cụ thể, cùng cơ chế Event-Contract-version-registry còn thiếu, VẪN là follow-on work riêng, ngoài phạm vi sửa đổi v0.6 này.

**Replay preparation contract cho evidence này:** resolve đúng artifact mà `computation_cursor` của fact đó tham chiếu → recompute content ID từ real bytes hiện tại → so sánh với evidence đã persist → fail closed TRƯỚC KHI Replay execution bắt đầu trên BẤT KỲ điều nào trong bốn trường hợp: missing artifact; malformed evidence; reference mismatch (evidence không tương ứng đúng fact's own `computation_cursor`); content-ID mismatch. Replay execution VẪN self-contained sau preparation — không thêm bất kỳ external resolution nào, giữ nguyên `EVID-05(a)`'s disposition đã validate (`tests/test_replay_isolation.py`), KHÔNG revisit. `computation_dependency_content_evidence` KHÔNG PHẢI một phần computation identity/dedup (§8b, không đổi, cùng discipline `computation_cursor`).
...
capability_id: feature-engineering
domain_context_id: feature-engineering
compatibility_commitment: backward_only   # ADR-038 (Approved), Chapter 10 §10.3.1 — explicit
                                           #   declaration; forward compatibility NOT committed
description: >
...
invariants:
  ...
  - "payload.computation_dependency_content_evidence PHẢI có mặt trên MỌI FeatureComputed — computation gốc lẫn replacement, KHÔNG NGOẠI LỆ — chứa đúng hai giá trị bắt buộc input_contract_content_id/stream_registry_content_id, sao chép NGUYÊN VĂN (verbatim) từ CÙNG một VerifiedInputContractAuthority instance mà computation engine đã resolve/cache cho CHÍNH computation này (v0.6, ADR-037 Approved) — KHÔNG được tính lại, phát minh, hay lấy từ một instance khác. Đây là artifact-content evidence, KHÔNG PHẢI một replay-cursor/ordering field."
  - "computation_dependency_content_evidence.input_contract_content_id/stream_registry_content_id PHẢI tương ứng quan hệ CHÍNH XÁC với computation_cursor.input_contract_ref/computation_cursor.stream_registry_version của ĐÚNG CÙNG fact đó (v0.6, ADR-037 Approved) — evidence mang content identity cho một input_contract_ref/stream_registry_version KHÁC với cursor của chính fact đó là malformed, PHẢI bị từ chối bất cứ đâu được validate, KHÔNG được chấp nhận chỉ vì cả hai field riêng lẻ well-formed."
  - "computation_dependency_content_evidence của một FeatureComputed replacement ĐỘC LẬP với evidence của fact nó supersede — mỗi computation, gốc hay replacement, pin đúng evidence CHÍNH XÁC nó dùng để evaluate, KHÔNG kế thừa/sao chép evidence của fact trước đó (v0.6, ADR-037 Approved), cùng discipline computation_cursor."
payload:
  ...
  computation_dependency_content_evidence:
    input_contract_content_id: {type: string, required: true, description: "SHA-256 content identity của đúng Input Contract artifact bytes mà computation_cursor.input_contract_ref của CHÍNH fact này đã resolve tới (v0.6, ADR-037 Approved)."}
    stream_registry_content_id: {type: string, required: true, description: "SHA-256 content identity của đúng Stream Registry artifact bytes mà computation_cursor.stream_registry_version của CHÍNH fact này đã resolve tới (v0.6, ADR-037 Approved)."}
```
...
invariants:
  ...
  - "payload.computation_dependency_content_evidence PHẢI có mặt trên MỌI FeatureFactInvalidated, KHÔNG NGOẠI LỆ (v0.6, ADR-037 Approved) — cùng shape/binding rule như §3: chứa input_contract_content_id/stream_registry_content_id sao chép nguyên văn từ CÙNG VerifiedInputContractAuthority instance mà evaluation dẫn tới invalidation này đã dùng, tương ứng quan hệ CHÍNH XÁC với computation_cursor (R_later) của CHÍNH FeatureFactInvalidated này — KHÔNG kế thừa/sao chép evidence của FeatureComputed đang bị invalidate, ĐỘC LẬP hoàn toàn, đối xứng invariant tương ứng ở §3."
payload:
  ...
  computation_dependency_content_evidence:
    input_contract_content_id: {type: string, required: true, description: "SHA-256 content identity của đúng Input Contract artifact bytes mà computation_cursor (R_later) của CHÍNH fact này đã resolve tới (v0.6, ADR-037 Approved)."}
    stream_registry_content_id: {type: string, required: true, description: "SHA-256 content identity của đúng Stream Registry artifact bytes mà computation_cursor (R_later) của CHÍNH fact này đã resolve tới (v0.6, ADR-037 Approved)."}
```
...
4. Latest valid revision
   S = revision hợp lệ MỚI NHẤT của swing_id đó, TRONG SỐ các revision full-cursor-visible
   tại R theo §12 (swing.md §1a sở hữu revision lineage/matching semantics — KHÔNG sở hữu
   Feature's computation-cursor visibility rule; xem quy ước dưới)

5. Not invalidated
   KHÔNG có SwingInvalidated full-cursor-visible tại R theo §12 cho ĐÚNG cặp (swing_id,
   swing_revision) của S (swing.md §5 sở hữu invalidation semantics — KHÔNG sở hữu Feature's
   computation-cursor visibility rule; xem quy ước dưới)
```

**Quy ước "visible tại R" xuyên suốt pipeline này (v0.5, đóng `P3-FEATURE-DC-A-MAJ-01`):** MỌI cụm "visible tại R" ở Bước 2, 4, 5 trên — và mọi tham chiếu ngắn gọn khác tới cùng khái niệm trong §9a — nghĩa là §12's full cursor visibility predicate (ba nhánh: stream-universe membership + in-stream sequence position + recorded-time boundary), KHÔNG PHẢI `swing.md` §7's recorded-time-only replay rule (`event.recorded_time <= T`) — hai quy tắc tách biệt kể từ v0.4/ADR-035, dù cùng phục vụ mục đích chống look-ahead. `swing.md` VẪN là authority DUY NHẤT cho Swing revision lineage (§1a), revision matching, và invalidation semantics (§5) — nó KHÔNG PHẢI authority cho Feature's computation-cursor visibility rule, vốn CHỈ định nghĩa tại §12. Ngoại lệ DUY NHẤT cho quy ước này: khi văn bản tường minh nói tới một trục thời gian khác (ví dụ `pivot_effective_time` ở Bước 3, hay `recorded_time` của chính correction trong đoạn "Correction-recorded-old-pivot" dưới đây — cả hai KHÔNG PHẢI computation-cursor visibility).
