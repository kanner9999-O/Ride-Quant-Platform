...
## Package 0.2-C4 — Trade Intent and Decision Foundation v0.2 (bounded correction applied, chưa `Consolidated Stable`, chờ delta Review A + Independent Review B)

**Phạm vi C4 (scope tối thiểu, đã Product Owner authorize):** hai Domain Contract — [`decision.md`](./decision.md) v0.2 Draft (Decision + DecisionEvaluationAttempt) và [`trade-intent.md`](./trade-intent.md) v0.2 Draft (Trade Intent) — HAI concept riêng biệt, HAI file riêng — `capability_id: decision-management` / `domain_context_id: strategy-decision` (đăng ký tại [`context-map.yaml`](./context-map.yaml) v0.14, không đổi trong bounded correction này).
...
- Bounded correction (v0.2): **hoàn tất** — cả 6 finding đã đóng, author self-review lần hai hoàn tất.
- ChatGPT delta Review A + Independent Review B (trên v0.2): **chưa chạy** — báo cáo correction transaction này gửi đi cho vòng delta review.
- Consolidation: **chưa bắt đầu** — Package 0.2-C4 **CHƯA** đạt `Consolidated Stable`.

**Không tuyên bố hoàn thành hay approval ở bất kỳ mức nào (mục C4):** `decision.md`/`trade-intent.md` `status: Draft`, `version: "0.2"`, `approved_by: null`, `approved_at: null`; không Product Owner Approve; không Lock; không Consolidate; không đóng OQ-002/OQ-003; không authorize Live; không sửa ADR-010/ADR-013 hay Constitution; Package 0.2-C1/C2/C3 vẫn `Consolidated Stable` và không đổi (`instrument.md`/`venue.md`/`account.md`/`strategy.md` byte-for-byte không đổi); Package 0.2-C5–C7 vẫn chưa authorize, chưa author.
