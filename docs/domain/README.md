...
**Package 0.2-C6 nay authored (chưa `Consolidated Stable`), xem mục "Package 0.2-C6" dưới đây.** Package 0.2-C7 vẫn chưa có artifact nào được author. OQ-002/OQ-003 vẫn `Open`. Không authorize Live ở bất kỳ hình thức nào. Phase 0.2 vẫn **active và chưa hoàn tất** — Package 0.2-A/B (tổng thể)/C1/C2/C3/C4/C5 nay đều `Consolidated Stable`, nhưng Phase 0.2 chỉ hoàn tất khi toàn bộ 0.2-C (C1–C7) cũng đạt tương đương, đúng roadmap Chapter 14.
...
## Package 0.2-C6 — Order Foundation v0.1 authored (chưa `Consolidated Stable`, chờ Review A + Independent Review B)

**Phạm vi C6 (scope tối thiểu, đã Product Owner authorize):** một Domain Contract — [`order.md`](./order.md) v0.1 Draft (Order + OrderCreationAttempt + OrderSubmissionRequest) — ba concept trong MỘT file, đúng quyết định tổ chức "prefer một bounded contract chứa cả creation và submission-request semantics" — `capability_id: execution-management` / `domain_context_id: order-management` (capability/context **MỚI**, đăng ký lần đầu tại [`context-map.yaml`](./context-map.yaml) v0.18 — Order KHÔNG thuộc phạm vi `risk-management`/`risk-gateway` của Package 0.2-C5). MỘT relationship edge MỚI đăng ký: `risk-gateway` (execution-intent) → `order-management` (order), `published_language`, contract `execution-intent-issued`.
...
- Consolidation: **chưa bắt đầu** — Package 0.2-C6 **CHƯA** đạt `Consolidated Stable`.

**Không tuyên bố hoàn thành hay approval ở bất kỳ mức nào (mục C6):** `order.md` `status: Draft`, `version: "0.1"`, `approved_by: null`, `approved_at: null`; không Product Owner Approve; không Lock; không Consolidate; không đóng OQ-002/OQ-003; không authorize Live; không sửa `execution-intent.md`/`risk.md`/`decision.md`/`trade-intent.md`/C1–C5/ADR/Constitution; Package 0.2-C1/C2/C3/C4/C5 vẫn `Consolidated Stable` và không đổi; Package 0.2-C7 vẫn chưa authorize, chưa author. KHÔNG Fill/Position semantics nào được author.
