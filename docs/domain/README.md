...
**Package 0.2-C5 — baseline dependency đã thỏa** (Trade Intent and Decision Foundation nay `Consolidated Stable`) — Product Owner đã authorize scope tối thiểu ("Package 0.2-C5 — Risk Gateway and Execution Intent Foundation v0.1"), xem mục "Package 0.2-C5" dưới đây cho chi tiết authoring. **Chưa Consolidated, chờ Review A + Independent Review B.**
...
## Package 0.2-C5 — Risk Gateway and Execution Intent Foundation v0.1 authored (chưa `Consolidated Stable`, chờ Review A + Independent Review B)

**Phạm vi C5 (scope tối thiểu, đã Product Owner authorize):** hai Domain Contract — [`risk.md`](./risk.md) v0.1 Draft (RiskEvaluation + RiskEvaluationAttempt) và [`execution-intent.md`](./execution-intent.md) v0.1 Draft (Execution Intent) — HAI concept riêng biệt, HAI file riêng, đúng yêu cầu "Risk and Execution Intent must remain separate concepts" — `capability_id: risk-management` / `domain_context_id: risk-gateway` (capability/context **MỚI**, đăng ký lần đầu tại [`context-map.yaml`](./context-map.yaml) v0.15 — Risk/Execution Intent KHÔNG thuộc phạm vi `decision-management`/`strategy-decision` của Package 0.2-C4).
...
**RiskEvaluation — tóm tắt (v0.1):** `risk_evaluation_id` (opaque, globally unique, immutable). Logical computation key `(trade_intent_id, risk_context_cursor)` — `risk_context_cursor` TÁI SỬ DỤNG nguyên vẹn Chapter 8 §8.5.1 Replay Cursor shape như PAYLOAD field (KHÔNG envelope-level ADR-010 field — `event_class: decision` KHÔNG áp dụng, chỉ riêng cho `decision.md`'s `DecisionRecorded`). `RiskEvaluationAttempt` (subject riêng): `attempt_outcome ∈ {EVALUATED, INELIGIBLE, FAILED_BEFORE_EVALUATION}` — MỌI lần thử ĐỀU ghi nhận, KHÔNG absence-based. Precondition: `eligible_for_new_risk_evaluation == true` (trade-intent.md §6a) — false → attempt INELIGIBLE, KHÔNG RiskEvaluation. Bốn trục risk evidence (đối xứng Strategy's bốn trục): `risk_policy_definition_version_ref`/`risk_policy_configuration_version_ref`/`risk_plugin_version_ref`/`package_build_artifact_ref`. Sizing bounded deterministic (`sizing_method: FIXED_RISK_BUDGET_NOTIONAL`) — năm check tuần tự (account active, environment=PAPER, sizing input valid, equity >= risk budget, projected exposure <= max notional) → `approved_quantity = FLOOR(configured_risk_budget / reference_price_value, quantity_precision)`. `result: APPROVED | REJECTED | NON_EVALUABLE` — bảy reason code đóng (`TRADE_INTENT_INELIGIBLE`, `ACCOUNT_NOT_ACTIVE`, `ENVIRONMENT_NOT_ALLOWED`, `RISK_BUDGET_EXCEEDED`, `REQUESTED_EXPOSURE_EXCEEDED`, `REQUIRED_EVIDENCE_UNAVAILABLE`, `INVALID_SIZING_INPUT`). Correction lineage: `risk_evaluation_id` bất biến per-fact, logical key hỗ trợ invalidate + same-key replacement (`supersedes_fact_ref`), visible-valid-head per logical key.

**Execution Intent — tóm tắt (v0.1):** `execution_intent_id` (opaque, globally unique, immutable), origin từ ĐÚNG MỘT RiskEvaluation APPROVED (`originating_risk_evaluation_id`, unique key, `execution_intent_derivation_idempotency_policy`). `account_id`/`instrument_selection_ref`/`direction`/`approved_quantity`/`quantity_unit` PHẢI khớp chính xác RiskEvaluation gốc — KHÔNG tự tính lại/thay đổi. `execution_action: OPEN_EXPOSURE` (v0.1, duy nhất). Lifecycle tối thiểu ba state (`ISSUED`/`WITHDRAWN`/`EXPIRED`), `supersedes_fact_ref` từ v0.1. Time ordering: `effective_time >= risk_evaluation_time`, `recorded_time > RiskEvaluationRecorded.recorded_time`. `eligible_for_new_order_creation` origin-validity rule cho C6 (chưa author) — khi RiskEvaluation gốc invalidate/supersede, Execution Intent liên quan mất eligibility cho Order creation mới mà KHÔNG tự động xóa/rewrite.

**Cardinality:** `RiskEvaluation.result = APPROVED` → zero hoặc một Execution Intent; `REJECTED`/`NON_EVALUABLE` → zero luôn luôn.

**Deferred limitations (Phase 1 implementation concern, non-blocking):** Stream Registry/Input Contract implementation cụ thể; nguồn cụ thể của evidence fact (Account/Ledger contract cho equity/exposure); `sizing_method` khác `FIXED_RISK_BUDGET_NOTIONAL`; correction lineage riêng cho `RiskEvaluationAttempt`; granular exception sub-taxonomy cho `FAILED_BEFORE_EVALUATION`; implementation technology cho RiskEvaluation→Execution Intent recovery; `environment = LIVE` policy support (v0.1 CHỈ PAPER); CLOSE/REDUCE execution_action; chính sách hết hạn Execution Intent cụ thể.
...
**Không tuyên bố hoàn thành hay approval ở bất kỳ mức nào (mục C5):** `risk.md`/`execution-intent.md` `status: Draft`, `version: "0.1"`, `approved_by: null`, `approved_at: null`; không Product Owner Approve; không Lock; không Consolidate; không đóng OQ-002/OQ-003; không authorize Live; không sửa `decision.md`/`trade-intent.md`/ADR-010/ADR-012/ADR-013 hay Constitution; Package 0.2-C1/C2/C3/C4 vẫn `Consolidated Stable` và không đổi (`instrument.md`/`venue.md`/`account.md`/`strategy.md`/`decision.md`/`trade-intent.md` byte-for-byte không đổi); Package 0.2-C6–C7 vẫn chưa authorize, chưa author.
