...
## Package 0.2-C7 — Execution Result, Fill, Position and Replay Integration Foundation v0.1 authored (chưa `Consolidated Stable`, chờ Review A + Independent Review B)

**Phạm vi C7 (scope tối thiểu, đã Product Owner authorize):** bốn Domain Contract — [`execution-result.md`](./execution-result.md) v0.1 Draft (ExecutionResult + ExecutionResultProcessingAttempt), [`fill.md`](./fill.md) v0.1 Draft (Fill), [`position.md`](./position.md) v0.1 Draft (Position, derived projection), [`replay-event.md`](./replay-event.md) v0.1 Draft (ReplayStateProjection, integration-only) — bốn context MỚI CÙNG capability `execution-management` đã có từ C6 (`execution-result-management`/`fill-management`/`position-management`/`replay-integration`, đăng ký tại [`context-map.yaml`](./context-map.yaml) v0.19). Mười relationship edge MỚI: `order-management`→`execution-result-management`, `execution-result-management`→`fill-management`, `fill-management`→`position-management`, cộng bảy edge `authoritative domain context`→`replay-integration` (strategy-decision×2, risk-gateway×2, order-management, execution-result-management, fill-management).

**Controlling architecture:** [`order.md`](./order.md) v0.2 Draft §8b (`eligible_for_execution_result_processing`, không sửa) + [Chapter 8 §8.1.1/§8.2/§8.5](../constitution/08-event-model.md) (Locked) + [Chapter 7 §7.4](../constitution/07-module-taxonomy.md) (Locked, Type 2 Projection cho Position) — không sửa/re-author/bump ADR/Constitution nào. Áp dụng lại bốn semantic pattern đã proven qua C4–C6 correction: KHÔNG circular reference Attempt↔ExecutionResult; attempt identity tách biệt logical result key; `attempt_outcome = PROCESSED` chỉ ghi sau khi computation hoàn tất trọn vẹn; `FAILED_BEFORE_RESULT` tường minh retryable.

**ExecutionResult — tóm tắt (v0.1):** `execution_result_id` (opaque, globally unique, immutable). **Logical result key = `submission_request_id` (KHÔNG `order_id`)** — đảm bảo một Submission Request bị invalidate/reissue KHÔNG BAO GIỜ nhầm lẫn ExecutionResult của request cũ sang request mới. `ExecutionResultProcessingAttempt` (subject riêng): `attempt_outcome ∈ {PROCESSED, INELIGIBLE, FAILED_BEFORE_RESULT}`. Precondition: `eligible_for_execution_result_processing == true` (order.md §8b, TRỌN VẸN năm điều kiện). `result_type ∈ {EXECUTED, NOT_EXECUTED}` — v0.1 KHÔNG venue-specific rejection taxonomy. Correction lineage: direct-predecessor-fact-targeting (đối xứng `risk.md` §10/`order.md` §9), CẶP BẮT BUỘC với Fill khi correction EXECUTED→NOT_EXECUTED (§7 execution-result.md).

**Fill — tóm tắt (v0.1):** `fill_id` (opaque, globally unique, immutable). Logical Fill key = `execution_result_id`. **v0.1 bounded rule (disclosed):** executed result sản sinh CHÍNH XÁC một full Fill, `fill_quantity == Order.quantity` — KHÔNG partial-fill semantics. `fill_price`/`price_currency` là giá PAPER execution quan sát được — KHÔNG limit price/slippage/price-guarantee model. Correction lineage direct-predecessor-fact-targeting, ràng buộc bắt buộc với ExecutionResult correction (Scenario 17).

**Position — tóm tắt (v0.1):** PROJECTION deterministic, non-authoritative, dẫn xuất TRỌN VẸN từ visible-valid Fill history (`fill.md` §5 fold) — KHÔNG authoritative event stream riêng, KHÔNG mutable command. Position key = (account_id, environment, instrument_selection_ref). `net_quantity` LUÔN non-negative magnitude, tách biệt khỏi `position_direction` (KHÔNG signed quantity). **v0.1 bounded rule (disclosed, §7 position.md):** đúng MỘT visible-valid Fill lineage đóng góp cho một Position key — tránh silently author portfolio netting/weighted-aggregation. KHÔNG compensating Position event khi Fill invalidate — recompute trực tiếp từ scratch.

**Replay Integration — tóm tắt (v0.1):** `ReplayStateProjection` — integration contract thuần túy, `ReplayState(C)` fold TOÀN BỘ tám thành phần (Decision/Trade Intent/RiskEvaluation/Execution Intent/Order/Submission Request/ExecutionResult/Fill lineage, cộng derived Position) tại MỘT canonical Replay Cursor duy nhất — KHÔNG author fact mới, KHÔNG duplicate authority (KHÔNG "ReplayDecision"/"ReplayOrder"/"ReplayFill" event nào). Tám cursor mốc bắt buộc (C0–C7) chứng minh trước/sau Submission Request, Execution Result, Fill, Fill invalidation, replacement Fill, Execution Result invalidation, replacement Execution Result.

**Deferred limitations (Phase 1 implementation concern, non-blocking):** Stream Registry/Input Contract implementation cụ thể; cơ chế/thuật toán PAPER simulation cụ thể xác định `result_type`/`fill_price`; granular exception sub-taxonomy cho `FAILED_BEFORE_RESULT`; correction lineage riêng cho `ExecutionResultProcessingAttempt`; partial-fill semantics (deferred hoàn toàn); multiple-Fill-per-Position-key weighted aggregation (deferred, v0.1 bound tới một Fill); Position cache/materialization cụ thể; implementation technology cho mọi recovery gap (Order→ExecutionResult, ExecutionResult→Fill); replay execution technology cụ thể.
...
**Không tuyên bố hoàn thành hay approval ở bất kỳ mức nào (mục C7):** `execution-result.md`/`fill.md`/`position.md`/`replay-event.md` `status: Draft`, `version: "0.1"`, `approved_by: null`, `approved_at: null`; không Product Owner Approve; không Lock; không Consolidate; không đóng OQ-002/OQ-003; không authorize Live; không sửa `order.md`/`execution-intent.md`/`risk.md`/`decision.md`/`trade-intent.md`/C1–C6/ADR/Constitution; Package 0.2-C1/C2/C3/C4/C5/C6 vẫn `Consolidated Stable` và không đổi. KHÔNG Live behavior, exchange adapter/API payload, fee/PnL/margin/leverage/liquidation semantics nào được author.
