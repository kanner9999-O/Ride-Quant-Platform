...
## Package 0.2-C7 — Execution Result, Fill, Position and Replay Integration Foundation v0.2 bounded correction applied (chưa `Consolidated Stable`, chờ bounded delta Review A + Independent Review B)

**Phạm vi C7 (scope tối thiểu, đã Product Owner authorize):** bốn Domain Contract — [`execution-result.md`](./execution-result.md) v0.2 Draft (PaperExecutionObservation + ExecutionResultProcessingAttempt + ExecutionResult), [`fill.md`](./fill.md) v0.2 Draft (Fill), [`position.md`](./position.md) v0.2 Draft (Position, derived projection), [`replay-event.md`](./replay-event.md) v0.2 Draft (ReplayStateProjection, integration-only) — bốn context (không đổi từ authoring) CÙNG capability `execution-management` (`execution-result-management`/`fill-management`/`position-management`/`replay-integration`, [`context-map.yaml`](./context-map.yaml) v0.19, không đổi trong bounded correction này — PaperExecutionObservation sống trong `execution-result.md`, cùng context, KHÔNG cần registration mới).
...
**v0.2 — bounded correction (đóng consolidated Review A + Independent Review B findings, bốn Major):** (1) `C7-MAJ-01` — thêm `PaperExecutionObservation` (entity MỚI, execution-result.md §1) — durable, immutable record của simulation evidence (policy/configuration/build/deterministic input refs) VÀ output; `ExecutionResult` nay CHỈ copy `result_type` từ Observation, KHÔNG tự computation; thứ tự corrected: computation → Observation → Attempt PROCESSED → Result; hai recoverable gap tường minh; correction EXECUTED↔NOT_EXECUTED dùng Observation MỚI tại cursor MỚI. (2) `C7-MAJ-02` — Fill economics nay BẮT BUỘC copy CHÍNH XÁC từ Observation (fill.md §1/§3, field MỚI `execution_observation_id`) — KHÔNG độc lập quan sát/recompute, kể cả khi recovery. (3) `C7-MAJ-03` — loại bỏ hoàn toàn ngôn ngữ "cặp bắt buộc"/"atomic-adjacent" giữa ExecutionResult và Fill correction — thay bằng continuing eligibility rule `eligible_as_position_contributing_fill` (fill.md §6, MỚI) đánh giá LIÊN TỤC tại mọi cursor, KHÔNG phụ thuộc thời điểm cleanup. (4) `C7-MAJ-04` — Position projection nay có `projection_status ∈ {EVALUABLE, NON_EVALUABLE}` (position.md §1/§2, MỚI) — khi > 1 eligible Fill lineage cùng Position key, projection trả `NON_EVALUABLE`/`UNSUPPORTED_MULTIPLE_FILL_LINEAGES` VÀ `contributing_fill_refs`, KHÔNG silently chọn/aggregate/report FLAT sai.

**ExecutionResult/PaperExecutionObservation — tóm tắt (v0.2):** `execution_observation_id` (opaque, immutable, logical computation key = `(submission_request_id, observation_cursor)` — đối xứng `risk.md`'s `(trade_intent_id, risk_context_cursor)`) — durable evidence (bốn trục: policy/configuration/build/deterministic-input refs) + output (`result_type`, `executed_quantity`, `execution_price`). `execution_result_id` (logical result key = `submission_request_id`, không đổi) — `result_type` COPY CHÍNH XÁC từ Observation. Correction EXECUTED↔NOT_EXECUTED dùng Observation MỚI tại cursor MỚI, KHÔNG invalidate Observation cũ (tránh excessive correction machinery).

**Fill — tóm tắt (v0.2):** `fill_id` (opaque, immutable). Logical Fill key = `execution_result_id` (không đổi). Economics (`fill_quantity`/`fill_price`/`price_currency`/`quantity_unit`) nay BẮT BUỘC = Observation economics CHÍNH XÁC, qua `execution_observation_id` field mới. `eligible_as_position_contributing_fill(fill_id, C)` (§6, MỚI) — continuing cursor-bound validity rule, nguồn sự thật DUY NHẤT cho Position, KHÔNG `FillCurrentView`.

**Position — tóm tắt (v0.2):** PROJECTION deterministic dẫn xuất từ `eligible_as_position_contributing_fill` (KHÔNG trực tiếp Fill stream). `projection_status ∈ {EVALUABLE, NON_EVALUABLE}` — zero eligible Fill → EVALUABLE FLAT; một → EVALUABLE LONG/SHORT; nhiều → NON_EVALUABLE, `UNSUPPORTED_MULTIPLE_FILL_LINEAGES`, `contributing_fill_refs` đầy đủ.

**Replay Integration — tóm tắt (v0.2):** `ReplayState(C)` nay fold THÊM `paper_execution_observation_lineage` VÀ `fill_continuing_eligibility` (tách biệt khỏi Fill stream riêng); `derived_position` phản ánh `projection_status`. Mười cursor mốc (C0–C9, thay tám mốc cũ) — bao gồm mốc "Result invalidation TRƯỚC Fill invalidation" (minh chứng `C7-MAJ-03`) VÀ mốc "nhiều eligible Fill lineage" (minh chứng `C7-MAJ-04`).

**Deferred limitations (Phase 1 implementation concern, non-blocking):** Stream Registry/Input Contract implementation cụ thể; cơ chế/thuật toán PAPER simulation cụ thể; granular exception sub-taxonomy; correction lineage riêng cho Attempt/Observation (append-only đủ, tránh excessive machinery); partial-fill semantics (deferred hoàn toàn); multiple-Fill-per-Position-key aggregation formula (deferred — v0.2 formally trả `NON_EVALUABLE` thay vì silently sai); Position cache/materialization cụ thể; implementation technology cho mọi recovery gap; replay execution technology cụ thể.
...
- ChatGPT bounded delta Review A + Independent Review B delta review (trên v0.2): **chưa chạy** — báo cáo correction transaction này gửi đi làm điểm khởi đầu delta review, đúng mandatory sequence: Author baseline → ChatGPT Review A → Independent Review B (cùng exact baseline) → merge finding → một correction commit được Product Owner authorize → ChatGPT delta review → Independent Review B delta review → Product Owner consolidation decision. **KHÔNG correction dựa trên một review đơn lẻ.**
- Consolidation: **chưa bắt đầu** — Package 0.2-C7 **CHƯA** đạt `Consolidated Stable`.

**Không tuyên bố hoàn thành hay approval ở bất kỳ mức nào (mục C7):** `execution-result.md`/`fill.md`/`position.md`/`replay-event.md` `status: Draft`, `version: "0.2"`, `approved_by: null`, `approved_at: null`; không Product Owner Approve; không Lock; không Consolidate; không đóng OQ-002/OQ-003; không authorize Live; không sửa `order.md`/`execution-intent.md`/`risk.md`/`decision.md`/`trade-intent.md`/C1–C6/ADR/Constitution; Package 0.2-C1/C2/C3/C4/C5/C6 vẫn `Consolidated Stable` và không đổi. KHÔNG Live behavior, exchange adapter/API payload, fee/PnL/margin/leverage/liquidation semantics nào được author. KHÔNG cross-stream atomic transaction nào được introduce.

## Package 0.2-C7 second bounded correction — v0.2 → v0.3 (đóng `C7-DELTA-MAJ-01`, chưa `Consolidated Stable`, chờ second bounded delta Review A + Independent Review B)

**Finding gốc:** logical computation key cũ `(submission_request_id, observation_cursor)` (`PaperExecutionObservation`, execution-result.md v0.2 §1) KHÔNG authoritatively phân biệt được initial computation / authorized correction computation / illegal rerun tại cursor mới / orphan Observation chờ Attempt — một cursor MỚI tự nhiên tạo một logical key MỚI mà KHÔNG chứng minh một computation MỚI thực sự được authorize.

**Sửa — `ExecutionResultComputation` (entity MỚI, execution-result.md §2):** identity authoritative đại diện ĐÚNG MỘT authorized computation lifecycle (`execution_result_computation_id`, opaque, KHÔNG derive từ scope). `computation_purpose ∈ {INITIAL, CORRECTION}`. **INITIAL:** tối đa MỘT computation cho mỗi `submission_request_id`, BẤT KỂ `computation_cursor` khác nhau — retry tại cursor khác KHÔNG tạo computation thứ hai, deterministic conflict (Scenario 32). **CORRECTION:** bắt buộc `predecessor_execution_result_ref` + `correction_authorization_ref` (trỏ `ExecutionResultFactInvalidated` targeting đúng predecessor), tối đa MỘT correction trực tiếp cho mỗi predecessor — cấm fork (Scenario 36). `ExecutionResultComputationAuthorized` (event MỚI, §5) là fact authoritative duy nhất thiết lập computation identity — "authorized" nghĩa domain eligibility (INITIAL)/correction lineage (CORRECTION) cho phép, KHÔNG phải Product Owner thủ công authorize từng runtime computation, KHÔNG workflow/saga infrastructure.

**`PaperExecutionObservation` logical identity đổi (§1):** nay = `execution_result_computation_id` (KHÔNG còn `(submission_request_id, cursor)`) — một computation → zero hoặc một Observation. Cùng computation + cùng cursor + cùng evidence/output → idempotent; cùng computation + cursor khác → deterministic conflict (Scenario 33); cùng computation + evidence/output khác → deterministic conflict (Scenario 25).

**Thứ tự corrected (§8a):** `ExecutionResultComputationAuthorized` → bounded simulation hoàn tất → `PaperExecutionObservationRecorded` → `ExecutionResultProcessingAttemptRecorded`(PROCESSED) → `ExecutionResultRecorded`. Attempt outcome ↔ computation/observation presence: `INELIGIBLE` — KHÔNG computation, KHÔNG observation (eligibility fail trước authorize); `FAILED_BEFORE_RESULT` — computation CÓ mặt (đã authorize), observation KHÔNG (simulation fail trước khi ghi); `PROCESSED` — cả hai CÓ mặt. Hai recoverable gap (Gap A: computation+Observation, Attempt absent; Gap B: +Attempt PROCESSED, Result absent) resolve BẰNG computation identity (`GetExecutionResultComputationById`/`GetObservationForComputation`) — KHÔNG search bằng `submission_request_id` đơn thuần, KHÔNG chọn Observation mới nhất, KHÔNG chọn cursor khác, KHÔNG rerun simulation (Scenario 4/B/E).

**Correction flow (§11):** E1 invalidated (I1) → K2 CORRECTION (predecessor=E1, correction_authorization_ref=I1) → O2 → A2 PROCESSED → E2 supersedes E1 (Scenario 34). Năm điều kiện reject bổ sung: correction không có invalidation hợp lệ, sai target, đổi `submission_request_id`, fork, target predecessor không phải current head (Scenario 35/37).

**`ExecutionResultRecorded`/Fill/Position không đổi logic:** `execution_result_computation_id` (MỚI, §8) BẮT BUỘC BẰNG HỆT Attempt/Observation; Result logical key VẪN `submission_request_id` (KHÔNG thay bằng computation identity). `fill.md`/`position.md` KHÔNG semantic change — Fill resolve computation identity TRANSITIVELY qua `execution_result_id`, KHÔNG field riêng. `C7-MAJ-02`/`C7-MAJ-03`/`C7-MAJ-04` KHÔNG reopen.

**Replay Integration (replay-event.md v0.3):** `ReplayState(C)` fold THÊM `execution_result_computation_lineage` (§2). Mười hai cursor mốc (C0–C11, thay mười mốc cũ) — chèn "sau computation authorization" (C1) và "sau correction computation authorized" (C8), minh chứng trực tiếp `C7-DELTA-MAJ-01`.

**Trạng thái review:** Author self-review (second bounded correction): hoàn tất — 37 tổng scenario (32–37 mới), cross-file citation script re-run, 0 collision. ChatGPT second bounded delta Review A + Independent Review B (trên v0.3): **chưa chạy** — báo cáo transaction này gửi đi làm điểm khởi đầu. Consolidation: **chưa bắt đầu**.

**Không tuyên bố hoàn thành hay approval nào (mục này):** `execution-result.md`/`replay-event.md` `version: "0.3"`; `fill.md`/`position.md` `version: "0.3"` (reference-consistency-only, KHÔNG semantic); tất cả `status: Draft`, `approved_by: null`, `approved_at: null`; không Approve/Lock/Consolidate; không đóng OQ-002/OQ-003; không authorize Live; không sửa `order.md`/`execution-intent.md`/`risk.md`/`decision.md`/`trade-intent.md`/`context-map.yaml`/C1–C6/ADR/Constitution; không author simulation algorithm thực tế, Live/adapter/partial-Fill/fees/PnL/accounting/margin/leverage/liquidation semantics, cross-stream transaction, hay workflow/saga infrastructure.
