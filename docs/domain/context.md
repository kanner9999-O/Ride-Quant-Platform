...
**Factual update (2026-09-26, artifact-existence status only — KHÔNG một normative change, KHÔNG reinterpret quyết định semantic-basis-acceptance của `CONTEXT-DOMAIN-V04-PO-ACCEPTANCE-001`).** Một candidate current/active Context-scoped Input Contract NAY đã tồn tại: [`docs/architecture/input-contracts/context-market-input.yaml`](../architecture/input-contracts/context-market-input.yaml), `version: "0.1"`, `status: Draft` (mutable current/active-path candidate theo [ADR-041](../adr/ADR-041.md) — KHÔNG Published, KHÔNG Approved). Identity Published canonical đầu tiên nó đề xuất là `input_contract_ref: {contract_id: context-market-input, contract_version: v1.0}` — CHƯA có immutable snapshot nào tồn tại tại `docs/architecture/input-contract-versions/context-market-input/v1.0.yaml`, nên identity đó CHƯA PHẢI một valid authoritative reference target. Authoritative Context publication do đó VẪN fail-closed đúng như tài liệu này/ADR-046 đã yêu cầu (§3, §4, §16 trên) — ghi chú này CHỈ cập nhật artifact cụ thể nào hiện đang thỏa nửa "schema tồn tại như một candidate" của fail-closed gate đó; nó KHÔNG đổi `version: "0.4"`/`status: Draft` của CHÍNH tài liệu này, KHÔNG đổi bất kỳ normative Context semantic nào, và KHÔNG tự nó authorize publication.
