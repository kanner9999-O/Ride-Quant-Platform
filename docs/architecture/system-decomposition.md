...
**CANDIDATE (package lifecycle, 2026-08-05, Package 1.1 v0.7 semantic alignment transaction) — artifact status: Draft, KHÔNG Approved/Locked.** Package 1.1 đảo package lifecycle từ `Consolidated Stable` (v0.6) → `candidate` (v0.7): `custody-signing-service.phase.elaborated_by` đổi `null` → `"1.2"` — MỘT semantic phase-assignment change (KHÔNG mechanical/wording-only), đúng tiền lệ v0.5's original registration và v0.2→v0.3 (ADR-016 alignment), cả hai cũng revert `Consolidated Stable` → `candidate`. v0.6's Consolidated Stable consolidation decision (2026-08-05T08:40:00+07:00, Product Owner) KHÔNG bị falsify — GIỮ NGUYÊN làm historical record cho đúng candidate v0.6's review-clean state tại thời điểm đó (xem lịch sử dưới đây); nó đơn giản KHÔNG còn LÀ current package lifecycle. v0.7 CẦN một bounded consistency verification pass VÀ một Product Owner consolidation decision MỚI trước khi trở lại `Consolidated Stable`. `status: Draft`, `approved_by: null`, `approved_at: null` KHÔNG đổi.
