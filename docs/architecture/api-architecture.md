...
Package 1.4:
  ...
  package lifecycle/readiness: candidate (reverted from Consolidated Stable, 2026-08-06
    — v0.6 ADR-020 parity-transcription transaction, xem banner đầu tài liệu)
  v0.6 KHÔNG reconsolidate — một Review A + Independent Review B + Product Owner
...
Package 1.4 v0.1 LÀ candidate đầu tiên — v0.2 LÀ bounded correction đóng ba Review A
  ...
  một bounded wording-only correction — reverts `package lifecycle` từ `Consolidated
