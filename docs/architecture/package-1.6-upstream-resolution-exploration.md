...
```text
Xác nhận tường minh (bằng chứng, KHÔNG quyết định): DD-001 (module-registry.yaml
notes, nguyên văn) NGĂN CHẶN chính xác MỘT điều — resolve `owns_authoritative_state`
của backtest-orchestrator thành `true`/`false` dứt khoát, tức là KHÔNG được claim
"Backtest run identity/context LÀ một authoritative domain fact được backtest-
orchestrator sở hữu" cho tới khi Product Owner author Backtest Domain Contract. DD-001
KHÔNG ngăn chặn, VÀ KHÔNG liên quan tới, việc `command-query-api-surface` có một
registered `depends_on` edge tới `backtest-orchestrator` hay không — routing/reachability
LÀ một field registry HOÀN TOÀN riêng biệt (`depends_on`), KHÔNG phụ thuộc giá trị
`owns_authoritative_state`. Bằng chứng hỗ trợ: `command-query-api-surface.depends_on`
(Package 1.4 v0.3) ĐÃ route tới `position-projection` (`owns_authoritative_state:
false`) VÀ `review-evidence-service` (`owns_authoritative_state: false`) — routing
edge KHÔNG BAO GIỜ đòi hỏi giá trị `owns_authoritative_state` cụ thể nào. KẾT LUẬN
(evidence-based, KHÔNG phải quyết định): thêm một `depends_on` edge tới backtest-
orchestrator, TỰ THÂN, KHÔNG resolve VÀ KHÔNG cần resolve DD-001 — hai vấn đề độc lập.
```
...
A. Thêm command-query-api-surface -> backtest-orchestrator dependency edge:
   Semantic fit:          CAO — khớp mẫu hình 16 edge hiện có (mọi backend module UX
                          cần đều route qua API Surface); khớp UC-006/007/010 nhu cầu
                          command (khởi tạo run)/query (trace/comparison).
   Authority effect:      KHÔNG — DD-001 KHÔNG resolve (§2.2); owns_authoritative_
                          state VẪN deferred.
   ...
   Package 1.1 hệ quả:    Registry content change → semantic (KHÔNG mechanical), đúng
                          ...
                          package_lifecycle revert Consolidated Stable → candidate,
                          cần MỘT vòng Review A + Independent Review B + Product
                          Owner consolidation decision MỚI.
   Package 1.4 hệ quả:    api-architecture.md §2.1 transcribe `depends_on` verbatim —
                          ...
                          1.4 correction transaction (parity update), khả năng revert
                          Consolidated Stable → candidate tương tự.
   ADR trigger:           Governance §4b — "Module Taxonomy/dependency graph" thay
                          đổi → ADR Required, KHÔNG mơ hồ.
   DD-001 preservation:   BẢO TOÀN — edge addition KHÔNG đóng DD-001.
   Rejection reason:      KHÔNG có lý do loại trừ tường minh từ evidence hiện tại —
                          NHƯNG chi phí thật: kích hoạt ADR + reopen HAI package đã
                          Consolidated Stable (1.1 VÀ 1.4).
...
NAV-003 edge addition (Candidate A, §2.4):
  ...
  Registry alignment:      CẦN — Package 1.1 correction transaction (thêm edge),
                           reverting Consolidated Stable → candidate, đúng tiền lệ
                           (custody-signing-service).
  Package lifecycle revert: CẦN cho CẢ Package 1.1 VÀ Package 1.4 (parity transcription
                           correction).
...
VIEW-002 computation ownership (NẾU assign review-evidence-service, §3.2):
  ...
  Package lifecycle revert: CẦN cho Package 1.1 VÀ Package 1.5.
...
VIEW-003 computation ownership (NẾU assign replay-integration-service HOẶC decision-
  ...
  Package lifecycle revert: CẦN cho Package 1.1 VÀ Package 1.3-A HOẶC Package 1.3-C
                           (BA package đang Consolidated Stable phải reopen — chi phí
                           cascading LỚN HƠN NAV-003/VIEW-002).
...
NAV-003 dependency edge:
  ADR REQUIRED — khớp CHÍNH XÁC §4b table entry "Module Taxonomy/dependency graph"
  thay đổi → "Bắt buộc." KHÔNG mơ hồ.

VIEW-002 responsibility/contract ownership:
  ADR CONDITIONALLY REQUIRED — phụ thuộc resolution path: NẾU cần một dependency edge
  MỚI (vd. review-evidence-service cần edge mới, hay API Surface cần một aggregation
  path mới) → ADR Required (cùng rule dependency-graph). NẾU CHỈ mở rộng
  `responsibilities` text của MỘT module hiện có, KHÔNG edge mới → về nguyên tắc CÓ
  THỂ rơi vào "ADR Optional" (§4b: "thay đổi nội bộ một module... ảnh hưởng đáng kể")
  NHƯNG §4b's tiêu chí thay thế "quyết định ảnh hưởng >1 module hoặc khó đảo ngược"
  RẤT CÓ THỂ vẫn kích hoạt (một ownership assignment cho computation cross-module ảnh
  hưởng downstream API/UX consumer) — phân loại BẢO THỦ tại đây: CONDITIONALLY
  REQUIRED, nghiêng về REQUIRED trong thực tế, phụ thuộc candidate cuối cùng được chọn.

VIEW-003 responsibility/module/contract ownership:
  ADR CONDITIONALLY REQUIRED (cùng lý luận VIEW-002) — NẾU chọn module MỚI → ADR
  Required dứt khoát (module taxonomy change, KHÔNG mơ hồ). NẾU mở rộng module hiện
  có (replay-integration-service/decision-evaluation-engine) → cùng phân loại bảo thủ
  như VIEW-002, nghiêng về REQUIRED do cascading cross-package impact (§5).
...
Xác nhận tường minh: BA quyết định này KHÔNG combine mặc định thành MỘT ADR duy nhất
  ...
  khác nhau, PHẢI đánh giá VÀ (nếu cần) author ADR riêng biệt, đúng yêu cầu task "Do
  not combine the decisions by default."
```
...
Lifecycle consequence:     Package 1.1 VÀ Package 1.4 revert Consolidated Stable →
                          candidate; Package 1.6 vẫn candidate cho tới khi binding
                          resolve.
Recommendation:            Evidence nghiêng nhẹ về Candidate A (semantic fit rõ ràng
                          nhất, chi phí đã biết VÀ bounded) — NHƯNG đây KHÔNG PHẢI
                          approval; câu hỏi §2.5 mục 2 (authoritative-fact ownership
                          của run-identity correlation) NÊN resolve trước khi tiến
                          hành ADR authoring, vì nó ảnh hưởng NỘI DUNG ADR đó.
...
Affected artifact (tiềm
  năng):                   use-case-workflow.md (UC-003 elaboration, nếu Product Owner
                          ...
                          architecture.md/ux-architecture.md (correction theo sau).
Lifecycle consequence:     Package 1.1 + package elaborate module được chọn revert
                          candidate; Package 1.6 vẫn blocked cho tới khi resolve.
...
Lifecycle consequence:     TIỀM NĂNG LỚN NHẤT trong ba candidate — có thể chạm Domain
                          ...
                          1.3-A hoặc 1.3-C + 1.4/1.5/1.6 theo sau).
...
Bước 2 — Product/Domain semantic amendment (CÓ ĐIỀU KIỆN, riêng biệt cho từng vấn
  đề):
  (a) NAV-003: KHÔNG cần — UC-006/007/010 đã đủ rõ cho mục đích binding.
...
Bước 3 — ADR authoring/review/approval (riêng biệt cho MỖI vấn đề, KHÔNG combine mặc
  định, §6): NAV-003 ADR (Required); VIEW-002 ADR (Conditionally Required, sau Bước
  2b); VIEW-003 ADR (Conditionally Required, sau Bước 2c).
...
Package 1.6 upstream-resolution exploration:
  ...
  pending Review A (bounded — xác nhận evidence trích xuất chính xác, KHÔNG một
    quyết định nào bị silently đưa ra dưới vỏ bọc "evidence")
