...
`publish.Memory` plays there, and the same role structure-engine's own
`publish.py` plays for that module: it assigns a contiguous, 1-based
sequence per `stream_id` and stamps a `ProducerRef`, but it is NOT a real
event log or broker, and it invents no stream-registry infrastructure.
Duplicated (not imported) from structure-engine — see `identity.py`'s module
docstring for why the two packages do not share code.
"""
