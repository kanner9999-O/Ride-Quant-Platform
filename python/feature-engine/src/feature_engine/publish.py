...
module-local stand-in — same role as structure-engine/raw-regime-engine's own
`publish.py`: it assigns a contiguous, 1-based sequence per `stream_id` and
stamps a `ProducerRef`, but it is NOT a real event log or broker, and it
invents no stream-registry infrastructure. Duplicated (not imported) — see
`identity.py`'s module docstring.
"""
